...
class MyLogger {
    ...
    static public function trace_to_array( $trace ) {
        $arr = [];
...
//                             $item['args'],
    ...
    /***
     * converts Exception to array tree.
     */        
    static public function exception_to_array( Exception $e ) {
        ...
        foreach( $exceptions as $e ) {
            $arr[] = ['message' => $e->getMessage(),
                      ...
                      'line'    => $e->getLine(),
//                      'trace'   => $e->getTrace(),
                      'trace'   => self::trace_to_array($e->getTrace()),
                     ];
        }
     ...
     * Logs an exception.
     */        
    public function log_exception_json( Exception $e, $log_level = mylogger::exception ) {
        $msg = json_encode( self::exception_to_array( $e ), JSON_PRETTY_PRINT);
        $this->log( $msg, $log_level );
        return $msg;
    }
     ...
     */            
    public function format_log_msg( $msg, $log_level, $time = null, $last_log = true ) {
        $time_buf = '';        
        if( $last_log ) {
            if( !$time ) {
                $time = microtime(true);
     ...
     * $log_level specifies the type/importance of the message.
     */            
    public function log( $msg, $log_level, $time = null ) {

        if( $log_level < $this->log_level ) {
            return;
        }
        $line = $this->format_log_msg( $msg, $log_level, $last_log = true, $time );
